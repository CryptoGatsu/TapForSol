...
app.use("/claim", (req: Request, res: Response, next: NextFunction) => {
  ...
  if (origin && !origin.includes("tapforsol.fun") && !origin.includes("github.io")) {
...
try {
  ...
  io.emit("new-claim", {
    wallet,
    amount: userReward / 1_000_000_000
  });

  await pool.query("INSERT INTO claims (wallet) VALUES ($1)", [wallet]);

  return res.json({ status: "paid", tx: signature, explorer: `https://solscan.io/tx/${signature}` });
